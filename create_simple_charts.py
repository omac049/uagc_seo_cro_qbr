#!/usr/bin/env python3
"""
Simple and reliable script to create interactive charts for the UAGC RFI Conversion Analysis Report
"""
...
def create_top_performers_chart(data):
    ...
    top_performers = data.nlargest(10, 'conversion_rate_change')
    bottom_performers = data.nsmallest(5, 'conversion_rate_change')
...
def update_html_with_charts():
    ...
    print("Replacing base64 images with interactive charts...")
    ...
    # Extract just the chart content from the HTML
    def extract_chart_content(html_string):
        if '<body>' in html_string and '</body>' in html_string:
            return html_string.split('<body>')[1].split('</body>')[0]
        return html_string
    
    # Replace first occurrence (executive summary)
    html_content = re.sub(
        base64_pattern,
        f'<div class="chart-container executive-summary">{extract_chart_content(executive_html)}</div>',
        html_content,
        count=1
    )
    
    # Replace second occurrence (top performers)
    html_content = re.sub(
        base64_pattern,
    ...
    # Replace third occurrence (distribution)
    html_content = re.sub(
        base64_pattern,
